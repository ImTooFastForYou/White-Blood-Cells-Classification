...
def extract_features_from_image(image_path, crop_params, thresholds):
    ...
    nuc_feat = pd.DataFrame(
        regionprops_table(
            nuc_clean, intensity_image=V, properties=props + ("mean_intensity",)
        )
    ).add_prefix("nuc_")
    cyto_feat = pd.DataFrame(
        regionprops_table(
            cyto_clean, intensity_image=V, properties=props + ("mean_intensity",)
        )
    ).add_prefix("cyto_")
...
if __name__ == "__main__":
    ...
    dataset_features.to_csv("features_XGBoost1.csv", index=False)
    dataset_features = pd.read_csv("features_XGBoost1.csv")
